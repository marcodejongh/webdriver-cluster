process.on('message', function(message) {
    ...
    var driver = new webdriver.Builder()
            .usingServer('http://nuew-apissd06.gfk.com:4448/wd/hub/')
    ...
    function done (e) {
        if (e) {
            result.push({ type: 'error', message: 'Step failed: ' + e.message});
        }
    }

    feature.scenarios.forEach(function(scenario) {
        flow.execute(function () {
            result.push({ type: 'scenario', message: 'Scenario: ' + scenario.title});
        });
        _.each(scenario.steps, function (step) {
            flow.execute(function () {
                result.push({ type: 'step', message: step});
